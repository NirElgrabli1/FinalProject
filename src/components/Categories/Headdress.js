...
import '../../App.css';
import {Box, Button, Center, Flex, GridItem, Heading, Image, Input, SimpleGrid, Spacer, Stack, Tag , Spinner} from "@chakra-ui/react";
...
const CheckApiReallength= CheckApiReal.length

const Headdress=[]
for (let i = 0; i < CheckApiReallength; i++){
      if (CheckApiReal[i].category=='hat'){
        Headdress.push(CheckApiReal[i]);
      }
}

useEffect(()=>{
    setLoading(false);
    SetStoreItem(Headdress);
    setFilteredItems(Headdress);
  },[]);
...
return (
    <>
     ...
     <Box bgColor='#f2f1f2' _active={scrollToTop}>
